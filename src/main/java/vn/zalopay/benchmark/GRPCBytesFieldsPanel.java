 ...
 */
public class GRPCBytesFieldsPanel extends JPanel implements ActionListener {
   ...
   // private static final String MIMETYPE = "send_file_mime_label"; //$NON-NLS-1$
    ...
    private void initializeTableModel() {
        tableModel = new ObjectTableModel(new String[] {  
                FIELDPATH, FILEPATH, OFFSET, READLENGTH},
            GRPCBytesField.class,
            new Functor[] {
                new Functor("getFieldPath"), //$NON-NLS-1$
                new Functor("getFilePath"), //$NON-NLS-1$
                new Functor("getOffset"), //$NON-NLS-1$
                new Functor("getReadLength")}, //$NON-NLS-1$
            new Functor[] {
                new Functor("setFieldPath"), //$NON-NLS-1$
    ...
    public static boolean testFunctors(){
        GRPCBytesFieldsPanel instance = new GRPCBytesFieldsPanel();
    ...
    public void modifyTestElement(TestElement testElement) {
        ...
        if (testElement instanceof GRPCSampler) { 
            GRPCSampler sampler = (GRPCSampler) testElement;
            @SuppressWarnings("unchecked") // we only put HTTPFileArgs in it
            Iterator<GRPCBytesField> modelData = (Iterator<GRPCBytesField>) tableModel.iterator();
            GRPCBytesFields fields = new GRPCBytesFields();
            while (modelData.hasNext()) {
                GRPCBytesField field = modelData.next();
                fields.addGRPCBinaryField(field); 
            }
            sampler.setBytesFields(fields); // find method in SamplerBase
        }
    }
    ...
    public void configure(TestElement testElement) {
        if (testElement instanceof GRPCSampler) {
            ...
            for(GRPCBytesField field : sampler.getBytesFields().asArray()){ 
    ...
    private void runCommandOnRow(String command, int rowSelected) {
        if (DELETE.equals(command)) {
            ...
            if(StringUtils.isNotBlank(path)) {
                tableModel.setValueAt(path, rowSelected, 1);
            }
        }
    }
    ...
    private void addFile() {
        ...
        tableModel.addRow(new GRPCBytesField());
        ...
        try {
            ...
            for (String clipboardLine : clipboardLines) {
                ...
                if (clipboardCols.length > 0) {
                    GRPCBytesField argument = createGRPCBinaryFieldFromClipboard(clipboardCols);
                    if (argument != null) {
    ...
    private GRPCBytesField createGRPCBinaryFieldFromClipboard(String[] clipboardCols) {
        if (clipboardCols.length == 1) {
            return new GRPCBytesField(clipboardCols[0], "");
        } else if (clipboardCols.length == 2) {
            return new GRPCBytesField(clipboardCols[0], clipboardCols[1]);
        } 
        return null;
    }
